...
void handle_int_increment(script * scr, char * in, char * tok1, char * tok2, char * gerr){
	in += next_word(in, tok2, tplussign);
...
void handle_setpixel(script * scr, char * in, char * tok1, char * tok2, char * gerr){
	...
	if(!advance){
		// Check if this an RGB var
		int success = 0;
		pix.rgb = get_rgb(scr, tok1, &success);
		if(success == -1){
			return; // Return with the error from next_rgb_set
		}
		*gerr = 0; // Disregard next_rgb_set error as this is a valid rgb variable
	}
...
void handle_rgb_declare(script * scr, char * in, char * tok1, char * tok2, char * gerr){
	...
	if(!valid_var_name(tok2)){
		sprintf(gerr, "\a\t\tInvalid variable name for type RGB on line %d '%s'",
			curr_script_line, tok2);
		return;
	}
	...
	get_int(scr, tok2, &success);
	if(success != -1){ // A variable by this name already exists
		sprintf(gerr, "\a\t\tRedeclaring RGB variable '%s' on line %d", tok2, curr_script_line);
		return ;
	}

	get_rgb(scr, tok2, &success);
	if(success != -1){
		sprintf(gerr, "\a\t\tRedeclaring RGB variable '%s' on line %d", tok2, curr_script_line);
		return ;
	}

	advance = next_word(in, tok1, topenbrace);
	
	COLORREF cref;
	if(*tok1 != '='){
		cref = default_rgb_val;
		if(!add_var_rgb(scr, tok2, cref)){
			sprintf(gerr, "\a\t\tToo many rgbs, rgb variables have a limit of %d", MAX_RGBS);
		}
		return ;
	}
	...
	in += advance;
	advance = 0;
	advance = next_rgb_set(scr, in, &cref, gerr);
	if(!advance){ // This is an assignment to another variable
		int success;
		get_rgb(scr, tok2, &success);
		if(success == -1){ // Invalid variable
			sprintf(gerr, "\a\t\tInvalid rvalue in assignment on line %d, no rgb variable by"
			" the name of '%s' previously assigned.", curr_script_line, tok1);
			return;
		}
		cref = scr->rgbs[success].cref ;
	}
